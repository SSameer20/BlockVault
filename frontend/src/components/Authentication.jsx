...
import { saveCredentials, getWalletData } from './Store'; // Import your store functions
...
export default function Authentication() {
    ...
    try {
      ...
      .then(store => {
        if (store && store.email && store.password) {
          ...
          if (data.email === store.email && store.password === btoa(data.password)) {
  ...
  const handleRegister = async (data) => {
    try {
      ...
      // Register the user via an API call
