  ...
  useEffect(() => {
    if (walletStore && walletStore.mnemonic) {
      axios.post("http://localhost:8080/create/wallet", {
        mnemonic: atob(walletStore.mnemonic) // Ensure this is Base64-encoded
      })
        .then((response) => {
      ...
      <Card className="w-2/5">
        <CardHeader className="flex justify-between">
          <select 
            ...
            onChange={(e) => setIdx(Number(e.target.value))} // Update index on selection change
          >
        ...
        <CardBody className='flex flex-col gap-5 py-10'>
          <CryptoCard name="Solana" address={solAccount[idx]} balance="0.0" symbol="SOL" />
          <CryptoCard name="Ethereum" address={ethAccount[idx]} balance="0.0" symbol="ETH" />
        </CardBody>
      </Card>
    </>
  );
